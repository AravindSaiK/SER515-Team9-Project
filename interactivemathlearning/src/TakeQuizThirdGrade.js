...
export default function TakeQuizThirdGrade(grade) {
    ...
    const takeQuiz = (quizName) => {
        ...
        axios.get('http://localhost:3001/takeQuiz',{
            ...
            .then(response =>  {
                if(response.data.data.quiz.grade == "Third Grade")
                    history.push("/AdvancedThird",response.data.data.quiz)
                else
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:3001/viewQuizzesThirdGrade')
            .then(response =>  {
